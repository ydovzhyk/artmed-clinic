    ...
    >
      {items.map((item) => (
        <Link
          key={item.href}
          href={item.href}
          className="rounded-full px-3 py-2 text-sm font-bold text-foreground-soft transition hover:bg-secondary-soft hover:text-primary xl:px-4"
        >
...
function MobileMenu({ open, onClose, items = [], languageSlot }) {
  const tCloseMenu = useTranslate('Закрыть меню')
          ...
          <nav aria-label={tMobileNavigation} className="flex flex-col gap-2">
            {items.map((item) => (
              <Link
                ...
                href={item.href}
                onClick={onClose}
                className="rounded-2xl border border-background-border bg-background-soft px-4 py-3 text-base font-bold text-foreground-soft transition hover:border-secondary/50 hover:bg-secondary-soft hover:text-primary"
              >
                <Text as="span" variant="body-sm">
                  {item.label}
                </Text>
